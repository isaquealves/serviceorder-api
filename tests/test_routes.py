...
def test_root(client, db):
    ...
    expected = {"message": "Welcome to Service order api"}
...
@pytest.mark.parametrize(
    "username, first, last, email, expected",
    (
...
def test_create_user_invalid_username(
    client, username, first, last, email, expected
):
...
def test_account_activation(client):
    ...
    link = "".join([f'/{config("API_VERSION", "v1")}/', f"users/activate/"])
    query = {"code": f"{user_id.decode()}+{code}"}

    response = client.get(
        link, query_string=query, content_type="application/json"
    )
...
def test_refresh_requires_jwt_token(client):
    with patch("app.api.authentication.jwt_refresh_token_required") as jwt:
        response = client.post(
            "/v1/auth/refresh",
            content_type="application/json",
            json=json.dumps({"refresh": "yes"}),
        )
        assert jwt.called_once()
        response_data = response.json.keys()
        assert all(lambda x: x in response_data for x in ["token", "refresh"])
