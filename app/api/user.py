...
def create(body: dict) -> Any:
    try:
        ...
        return bad_request_response(
            "User", "Data provided is not serializable"
        )
...
def activate(code):
    ...
    # fmt: off
    cyphered_data = urllib.parse.unquote_to_bytes(
        code[code.index(sep) + 1:]
    )
    # fmt: on
    ...
    return bad_request_response("User", "Requests data is not recognized")
